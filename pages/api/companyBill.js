...
import companyBillSchema from "../../mongodb/Schema/companyBillSchema"

const handler = async (req, res) => {
    if(req.method=="POST"){
        try {
            const record = new companyBillSchema(req.body);
        ...
        if(req.headers.query!=undefined){
          try{
            ...
            if(d.lastRecord!=undefined){
              const bills= await companyBillSchema.find().limit(d.lastRecord).sort({$natural:-1}) 
            return res.status(200).send(bills)
            }
            const bills= await companyBillSchema.find(d)
            return res.status(200).send(bills);
          }
          catch{
        ...
        const number= await companyBillSchema.count()
        return res.status(200).send(number);
      }

      else if (req.method === 'PUT') {
        try {
          const billUpdate = await companyBillSchema.findOneAndUpdate({billNo:req.body.billNo},req.body);
          return res.status(200).send({message:"Bill updated Sucessfully", recorded:true});
        } 
        catch (error) {
